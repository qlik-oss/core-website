...
| -------- | ----------- |
| filename | The name of a media file including path, if needed, as a folder data connection.<BR>Example: 'lib://Table Files/'<BR>In legacy scripting mode, the following path formats are also supported:<BR>- absolute: Example: c:\data\ <BR> - relative to the Qlik Sense app working directory: Example: data\ |
